...
const FinanceCore = {
    ...
    schema: `
        ...
        CREATE TABLE IF NOT EXISTS accounts (id TEXT PRIMARY KEY, name TEXT NOT NULL, icon TEXT, color TEXT, is_active INTEGER NOT NULL DEFAULT 1);
        CREATE TABLE IF NOT EXISTS periods (id TEXT PRIMARY KEY, name TEXT NOT NULL, description TEXT, start_date TEXT NOT NULL, end_date TEXT, is_open INTEGER NOT NULL DEFAULT 1, close_description TEXT, created_at TEXT NOT NULL);
        CREATE TABLE IF NOT EXISTS categories (id TEXT PRIMARY KEY, name TEXT NOT NULL);
        CREATE TABLE IF NOT EXISTS subcategories (id INTEGER PRIMARY KEY AUTOINCREMENT, category_id TEXT NOT NULL REFERENCES categories(id) ON DELETE CASCADE, name TEXT NOT NULL);
        CREATE TABLE IF NOT EXISTS entries (id INTEGER PRIMARY KEY, account_id TEXT NOT NULL REFERENCES accounts(id) ON DELETE CASCADE, period_id TEXT REFERENCES periods(id) ON DELETE SET NULL, type TEXT NOT NULL, value REAL NOT NULL DEFAULT 0, description TEXT, category TEXT, notes TEXT, date TEXT NOT NULL, created_at TEXT NOT NULL);
        CREATE TABLE IF NOT EXISTS investments (id INTEGER PRIMARY KEY, account_id TEXT NOT NULL REFERENCES accounts(id) ON DELETE CASCADE, value REAL NOT NULL DEFAULT 0, type TEXT, member TEXT, date TEXT NOT NULL);
    `,
    ...
    async load() {
            ...
            try {
                bytes = new Uint8Array(
                    await Neutralino.filesystem.readBinaryFile(
                        `${window.NL_DATAPATH || "."}/finance.sqlite`,
                    ),
                );
            } catch {
                bytes = null;
        ...
        if (stored) this.data = stored;
            ...
            const legacy = this.readState() || (await this.readLegacyData());
            this.data = legacy || this.createInitialData();
    ...
    async readLegacyData() {
        try {
            const content = await Neutralino.storage.getData(
